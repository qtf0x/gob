	...
	MIN=144871.4  1/m^2	MAX=492170 1/m^2	*/
			...
			if (x_loc > panelwidth) {
				...
				if (y_loc < 0) {
					...
					VSI = SUB_CRITICAL_Trona_STARTUP_CORNER(
					...
					x_loc_norm =
						(-(x_loc - box[1]) / (box[1]));
					y_loc_norm = (y_loc / box[5]);
					FUN2 = SUB_CRITICAL_Trona_CENTER_GATEROAD(
						x_loc_norm);
					FUN1 = SUB_CRITICAL_Trona_STARTUP_CORNER(
						x_loc_norm, y_loc_norm);
					VSI = (FUN1 * (mix) + FUN2 * (1 - mix));
				} else if ((y_loc <
					    (box[4] - blendrangey - 20)) &&
					   (y_loc > (box[3] + blendrangey))) {
					x_loc_norm =
						(-(x_loc - box[1]) / (box[1]));
					VSI = SUB_CRITICAL_Trona_CENTER_GATEROAD(
						x_loc_norm);
				} else if ((y_loc <
					    (box[4] + blendrangey + 20)) &&
					   (y_loc >
					...
					x_loc_norm =
						(-(x_loc - box[1]) / (box[1]));
					FUN1 = SUB_CRITICAL_Trona_CENTER_GATEROAD(
						x_loc_norm);
					...
					y_loc_norm = (-(y_loc - box[5]) /
						      (box[5] - box[4])) +
						     .012;
					FUN2 = SUB_CRITICAL_Trona_RECOVERY_CORNER(
						x_loc_norm, y_loc_norm);
					...
					y_loc_norm = (-(y_loc - box[5]) /
						      (box[5] - box[4])) +
						     .012;
					VSI = SUB_CRITICAL_Trona_RECOVERY_CORNER(
						x_loc_norm, y_loc_norm);
				} else {
					VSI = 0;
				}

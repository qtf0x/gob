...
DEFINE_ON_DEMAND(VSI_MINE_Trona_Stepped)
{
	...
	if (RP_Variable_Exists_P(
		    "vsi/panel-width")) { /* Returns true if the variable exists */
	...
	/* UPDATE VALUES */
	x_loc = fabs(x[0] -
		     panelxoffset); /* Center of Panel is Zero and Mirrored */
	...
	if (x_loc > panelwidth) {
		...
		if (y_loc < 0) {
			...
			x_loc_norm = (-(x_loc - box[1]) /
				      (box[1])); /* NORMALIZE to equation */
			y_loc_norm = (y_loc / box[5]);
			VSI = SUB_CRITICAL_Trona_STARTUP_CORNER(x_loc_norm,
								y_loc_norm);
		} else if ((y_loc < (box[3] + blendrangey)) &&
			   (y_loc > (box[3] - blendrangey))) {
			mix = -((y_loc - box[3] - blendrangey) /
				(2 * blendrangey));
			x_loc_norm = (-(x_loc - box[1]) / (box[1]));
			y_loc_norm = (y_loc / box[5]);
			FUN2 = SUB_CRITICAL_Trona_CENTER_GATEROAD(x_loc_norm);
			FUN1 = SUB_CRITICAL_Trona_STARTUP_CORNER(x_loc_norm,
								 y_loc_norm);
			VSI = (FUN1 * (mix) + FUN2 * (1 - mix));
		} else if ((y_loc < (box[4] - blendrangey - 20)) &&
			   (y_loc > (box[3] + blendrangey))) {
			x_loc_norm = (-(x_loc - box[1]) / (box[1]));
			VSI = SUB_CRITICAL_Trona_CENTER_GATEROAD(x_loc_norm);
		} else if ((y_loc < (box[4] + blendrangey + 20)) &&
			   (y_loc > (box[4] - blendrangey - 20))) {
			x_loc_norm = (-(x_loc - box[1]) / (box[1]));
			FUN1 = SUB_CRITICAL_Trona_CENTER_GATEROAD(x_loc_norm);
			x_loc_norm = (-(x_loc - box[1]) / (box[1] + 40)) + 0.02;
			y_loc_norm =
				(-(y_loc - box[5]) / (box[5] - box[4])) + .012;
			FUN2 = SUB_CRITICAL_Trona_RECOVERY_CORNER(x_loc_norm,
								  y_loc_norm);
			mix = -((y_loc - box[4] - blendrangey - 20) /
				(2 * blendrangey + 40));
			VSI = (FUN1 * (mix) + FUN2 * (1 - mix));
		} else if ((y_loc < box[5]) &&
			   (y_loc >
			    (box[4] + blendrangey +
			     20))) { /* Remained of data points are in the recovery room 600-1000m */
			x_loc_norm = (-(x_loc - box[1]) / (box[1] + 40)) + 0.02;
			y_loc_norm =
				(-(y_loc - box[5]) / (box[5] - box[4])) + .012;
			VSI = SUB_CRITICAL_Trona_RECOVERY_CORNER(x_loc_norm,
								 y_loc_norm);
		} else {
			VSI = 0;
		}
	...
	C_UDMI(c, t, 4) =
		(VSI > maximum_vsi) ?
			maximum_vsi :
			VSI; /* Assign VSI to user-defined-memory location after cropping to maximum change */
}
end_c_loop(c, t)
}
	...
	if (x_loc > panelwidth) {
		...
		if (x_loc < (box[1] - blendrange)) {
			if (y_loc < 0) {
				...
				VSI = SUPER_CRITICAL_MINE_C_STARTUP_CENTER(
					x_loc_norm, y_loc_norm);
			} else if ((y_loc > (box[4] - blendrangey - 15)) &&
				   (y_loc < box[4] + blendrangey - 15)) {
				mix = ((y_loc - blendrangey - 15) /
				       (2 * blendrangey + 30));
				x_loc_norm =
					(-(x_loc - box[1]) /
					 (box[1])); /* NORMALIZE to equation */
				y_loc_norm = (box[4]);
				FUN1 = SUPER_CRITICAL_MINE_C_STARTUP_CENTER(
					x_loc_norm, y_loc_norm);
				x_loc_norm = (-(x_loc - box[1]) / (box[1]));
				y_loc_norm =
					((y_loc - box[4]) / (box[5] - box[4]));
				FUN2 = SUPER_CRITICAL_MINE_C_CENTER_PANEL(
					x_loc_norm, y_loc_norm);
				VSI = (FUN2 * (mix) + FUN1 * (1 - mix));
			} else if ((y_loc < (box[5] - blendrangey - 15)) &&
				   (y_loc > (box[4] + blendrangey - 15))) {
				x_loc_norm =
					(-(x_loc - box[1] + 10) / (box[1]));
				y_loc_norm =
					((y_loc - box[4]) / (box[5] - box[4]));
				VSI = SUPER_CRITICAL_MINE_C_CENTER_PANEL(
					x_loc_norm, y_loc_norm);
			} else if ((y_loc > (box[5] - blendrangey - 15)) &&
				   (y_loc < box[5] + blendrangey + 15)) {
				mix = -((y_loc - box[5] - blendrangey - 15) /
					(2 * blendrangey + 30));
				x_loc_norm = (-(x_loc - box[1]) / (box[1]));
				y_loc_norm = ((y_loc - box[4] - 100) /
					      (box[5] - box[4]));
				FUN1 = SUPER_CRITICAL_MINE_C_CENTER_PANEL(
					x_loc_norm, y_loc_norm);
				x_loc_norm =
					((x_loc - box[1] + blendrange + 15) /
					 (box[1]));
				y_loc_norm = (1 - (y_loc - box[5]) /
							  (box[6] - box[5]));
				FUN2 = SUPER_CRITICAL_MINE_C_RECOVERY_CENTER(
					x_loc_norm, y_loc_norm);
				VSI = (FUN1 * (mix) + FUN2 * (1 - mix));
			} else if ((y_loc < box[6]) &&
				   (y_loc >
				...
				y_loc_norm = (1 - (y_loc - box[5]) /
							  (box[6] - box[5]));
				VSI = SUPER_CRITICAL_MINE_C_RECOVERY_CENTER(
					x_loc_norm, y_loc_norm);
			} else {
				VSI = 0;
			}
		} else if (x_loc <= (box[1] + blendrange)) {
			mix = ((x_loc - box[1] + blendrange) /
			       (2 * blendrange));
			if (y_loc < 0) {
				VSI = 0;
			} else if (y_loc < box[4]) {
				...
				FUN1 = SUPER_CRITICAL_MINE_C_STARTUP_CENTER(
					x_loc_norm, y_loc_norm);
				x_loc_norm = (1 - (x_loc - box[1]) /
							  (box[2] - box[1]));
				y_loc_norm = (y_loc / box[4]);
				FUN2 = SUPER_CRITICAL_MINE_C_STARTUP_GATEROADS(
					x_loc_norm, y_loc_norm);
				VSI = (FUN2 * (mix) + FUN1 * (1 - mix));
			} else if (y_loc <= box[5]) {
				x_loc_norm = (-(x_loc - box[1]) / (box[1]));
				...
				y_loc_norm =
					((y_loc - box[4]) / (box[5] - box[4]));
				FUN2 = SUPER_CRITICAL_MINE_C_CENTER_GATEROADS(
					x_loc_norm, y_loc_norm);
				VSI = (FUN2 * (mix) + FUN1 * (1 - mix));
			} else if (y_loc < box[6]) {
				x_loc_norm = ((x_loc - (box[1] - blendrange)) /
					      (box[1] + blendrange));
				y_loc_norm = (1 - (y_loc - box[5]) /
							  (box[6] - box[5]));
				FUN1 = SUPER_CRITICAL_MINE_C_RECOVERY_CENTER(
					x_loc_norm, y_loc_norm);
				x_loc_norm = (1 - (x_loc - (box[1])) /
							  (box[2] - box[1]));
				y_loc_norm = (1 - (y_loc - box[5]) /
							  (box[6] - box[5]));
				FUN2 = SUPER_CRITICAL_MINE_C_RECOVERY_GATEROADS(
					x_loc_norm, y_loc_norm);
				VSI = (FUN2 * (mix) + FUN1 * (1 - mix));
			} else {
				VSI = 0;
			...
			if (y_loc < 0) {
				...
				x_loc_norm = (1 - (x_loc - box[1]) /
							  (box[2] - box[1]));
				y_loc_norm = (y_loc / box[4]);
				VSI = SUPER_CRITICAL_MINE_C_STARTUP_GATEROADS(
					x_loc_norm, y_loc_norm);
			} else if ((y_loc > (box[4] - blendrangey)) &&
				   (y_loc < box[4] + blendrangey)) {
				mix = ((y_loc - box[4] + blendrangey) /
				       (2 * blendrangey));
				x_loc_norm = (1 - (x_loc - box[1]) /
							  (box[2] - box[1]));
				y_loc_norm = (y_loc / box[4]);
				FUN1 = SUPER_CRITICAL_MINE_C_STARTUP_GATEROADS(
					x_loc_norm, y_loc_norm);
				x_loc_norm = (1 - (x_loc - box[1]) /
							  (box[2] - box[1]));
				y_loc_norm =
				...
				y_loc_norm =
					((y_loc - box[4]) / (box[5] - box[4]));
				VSI = SUPER_CRITICAL_MINE_C_CENTER_GATEROADS(
					x_loc_norm, y_loc_norm);
			} else if ((y_loc > (box[5] - blendrangey - 20)) &&
				   (y_loc < box[5] + blendrangey + 20)) {
				mix = ((y_loc - box[5] + blendrangey) /
				       (2 * blendrangey));
				x_loc_norm = (1 - (x_loc - box[1]) /
							  (box[2] - box[1]));
				y_loc_norm =
					((y_loc - box[4]) / (box[5] - box[4]));
				FUN1 = SUPER_CRITICAL_MINE_C_CENTER_GATEROADS(
					x_loc_norm, y_loc_norm);
				x_loc_norm = (1 - (x_loc - box[1]) /
							  (box[2] - box[1]));
				y_loc_norm = (1 - (y_loc - box[5]) /
				...
				y_loc_norm = (1 - (y_loc - box[5]) /
							  (box[6] - box[5]));
				VSI = SUPER_CRITICAL_MINE_C_RECOVERY_GATEROADS(
					x_loc_norm, y_loc_norm);
			} else {
				VSI = 0;
			}
		...
		if (x_loc < (box[1] - blendrange)) {
			if (y_loc < 0) {
				...
				x_loc_norm =
					(-(x_loc - box[1] + 20) /
					 (box[1])); /* NORMALIZE to equation */
				y_loc_norm = (y_loc / box[4]);
				VSI = SUPER_CRITICAL_MINE_E_STARTUP_CENTER(
					x_loc_norm, y_loc_norm);
			} else if ((y_loc > (box[4] - blendrangey - 15)) &&
				   (y_loc < box[4] + blendrangey - 15)) {
				mix = ((y_loc - (box[4] + blendrangey - 15)) /
				       (2 * blendrangey));
				x_loc_norm =
					(-(x_loc - box[1] + 20) /
					 (box[1])); /* NORMALIZE to equation */
				y_loc_norm = (y_loc / box[4]);
				FUN1 = SUPER_CRITICAL_MINE_E_STARTUP_CENTER(
					x_loc_norm, y_loc_norm);
				x_loc_norm =
					(-(x_loc - box[1] + 10) / (box[1]));
				y_loc_norm =
					((y_loc - box[4]) / (box[5] - box[4]));
				FUN2 = SUPER_CRITICAL_MINE_E_CENTER_PANEL(
					x_loc_norm, y_loc_norm);
				VSI = (FUN2 * (mix) + FUN1 * (1 - mix));
			} else if ((y_loc < (box[5] - blendrangey - 15)) &&
				   (y_loc > (box[4] + blendrangey - 15))) {
				...
				y_loc_norm =
					((y_loc - box[4]) / (box[5] - box[4]));
				VSI = SUPER_CRITICAL_MINE_E_CENTER_PANEL(
					x_loc_norm, y_loc_norm);
			} else if ((y_loc > (box[5] - blendrangey - 15)) &&
				   (y_loc < box[5] + blendrangey - 15)) {
				mix = -((y_loc - (box[5] + blendrangey - 15)) /
				...
				y_loc_norm =
					((y_loc - box[4]) / (box[5] - box[4]));
				FUN1 = SUPER_CRITICAL_MINE_E_CENTER_PANEL(
					x_loc_norm, y_loc_norm);
				x_loc_norm =
					((x_loc - box[1] + blendrange + 15) /
					 (box[1]));
				y_loc_norm = (1 - (y_loc - box[5]) /
							  (box[6] - box[5]));
				FUN2 = SUPER_CRITICAL_MINE_E_RECOVERY_CENTER(
					x_loc_norm, y_loc_norm);
				VSI = (FUN1 * (mix) + FUN2 * (1 - mix));
			} else if ((y_loc < box[6]) &&
				   (y_loc >
				...
				y_loc_norm = (1 - (y_loc - box[5]) /
							  (box[6] - box[5]));
				VSI = SUPER_CRITICAL_MINE_E_RECOVERY_CENTER(
					x_loc_norm, y_loc_norm);
			} else {
				VSI = 0;
			}
			...
			if (y_loc < 0) {
				...
				x_loc_norm = (1 - (x_loc - box[1]) /
							  (box[2] - box[1]));
				y_loc_norm = (y_loc / box[4]);
				FUN2 = SUPER_CRITICAL_MINE_E_STARTUP_GATEROADS(
					x_loc_norm, y_loc_norm);
				VSI = (FUN2 * (mix) + FUN1 * (1 - mix));
			} else if (y_loc <= box[5]) {
				x_loc_norm = (-(x_loc - box[1]) / (box[1]));
				...
				y_loc_norm =
					((y_loc - box[4]) / (box[5] - box[4]));
				FUN2 = SUPER_CRITICAL_MINE_E_CENTER_GATEROADS(
					x_loc_norm, y_loc_norm);
				VSI = (FUN2 * (mix) + FUN1 * (1 - mix));
			} else if (y_loc < box[6]) {
				x_loc_norm = ((x_loc - (box[1])) / (box[1]));
				y_loc_norm = (1 - (y_loc - box[5]) /
							  (box[6] - box[5]));
				FUN1 = SUPER_CRITICAL_MINE_E_RECOVERY_CENTER(
					x_loc_norm, y_loc_norm);
				x_loc_norm = (1 - (x_loc - (box[1])) /
							  (box[2] - box[1]));
				y_loc_norm = (1 - (y_loc - box[5]) /
							  (box[6] - box[5]));
				FUN2 = SUPER_CRITICAL_MINE_E_RECOVERY_GATEROADS(
					x_loc_norm, y_loc_norm);
				VSI = (FUN2 * (mix) + FUN1 * (1 - mix));
			} else {
				VSI = 0;
			...
			if (y_loc < 0) {
				...
				x_loc_norm = (1 - (x_loc - box[1]) /
							  (box[2] - box[1]));
				y_loc_norm = (y_loc / box[4]);
				VSI = SUPER_CRITICAL_MINE_E_STARTUP_GATEROADS(
					x_loc_norm, y_loc_norm);
			} else if ((y_loc > (box[4] - blendrangey)) &&
				   (y_loc < (box[4] + blendrangey))) {
				mix = ((y_loc - box[4] + blendrangey) /
				...
				x_loc_norm = (1 - (x_loc - box[1]) /
							  (box[2] - box[1]));
				y_loc_norm = (y_loc / box[4]);
				FUN1 = SUPER_CRITICAL_MINE_E_STARTUP_GATEROADS(
					x_loc_norm, y_loc_norm);
				x_loc_norm = (1 - (x_loc - box[1]) /
							  (box[2] - box[1]));
				y_loc_norm =
				...
				y_loc_norm =
					((y_loc - box[4]) / (box[5] - box[4]));
				VSI = SUPER_CRITICAL_MINE_E_CENTER_GATEROADS(
					x_loc_norm, y_loc_norm);
			} else if ((y_loc > (box[5] - blendrangey - 20)) &&
				   (y_loc < box[5] + blendrangey + 20)) {
				mix = ((y_loc - box[5] + blendrangey) /
				...
				y_loc_norm =
					((y_loc - box[4]) / (box[5] - box[4]));
				FUN1 = SUPER_CRITICAL_MINE_E_CENTER_GATEROADS(
					x_loc_norm, y_loc_norm);
				x_loc_norm = (1 - (x_loc - box[1]) /
							  (box[2] - box[1]));
				y_loc_norm = (1 - (y_loc - box[5]) /
				...
				y_loc_norm = (1 - (y_loc - box[5]) /
							  (box[6] - box[5]));
				VSI = SUPER_CRITICAL_MINE_E_RECOVERY_GATEROADS(
					x_loc_norm, y_loc_norm);
			} else {
				VSI = 0;
			}
...
DEFINE_ON_DEMAND(VSI_MINE_Worrall_C)
{
	...
	thread_loop_c(t, d)
	{
		...
		{
			C_CENTROID(x, c, t);
			x_loc = (fabs(x[0] - panelxoffset) / panelwidth *
				 166.88) -
				9.625; /* headgate 147.63, tailgate at 166.88  */
			y_loc = ((x[1] - panelyoffset) / (panelength / 2) *
				 1474.3) +
				1465.65; /* recovery 1474.3, startup at -1457 */
			if ((fabs(x[0] - panelxoffset) < panelwidth) &&
			    (fabs(y_loc) < 1474.3)) {
				/*WORRALL: Scales correctly */
				cellpor = pow(x_loc, 1) * pow(y_loc, 9) *
						  (-4.11E-36) +
					  pow(x_loc, 0) * pow(y_loc, 10) *
						  (6.35E-32) +
					  pow(x_loc, 0) * pow(y_loc, 9) *
						  (-3.66E-29) +
					  pow(x_loc, 0) * pow(y_loc, 8) *
						  (-2.57E-25) +
					  pow(x_loc, 1) * pow(y_loc, 8) *
						  (-3.02E-29) +
					  pow(x_loc, 2) * pow(y_loc, 8) *
						  (-1.57E-30) +
					  pow(x_loc, 0) * pow(y_loc, 7) *
						  (1.32E-22) +
					  pow(x_loc, 1) * pow(y_loc, 7) *
						  (8.23E-27) +
					  pow(x_loc, 2) * pow(y_loc, 7) *
						  (4.25E-28) +
					  pow(x_loc, 3) * pow(y_loc, 7) *
						  (-1.54E-33) +
					  pow(x_loc, 0) * pow(y_loc, 6) *
						  (3.95E-19) +
					  pow(x_loc, 1) * pow(y_loc, 6) *
						  (1.16E-22) +
					  pow(x_loc, 2) * pow(y_loc, 6) *
						  (6.00E-24) +
					  pow(x_loc, 3) * pow(y_loc, 6) *
						  (-3.85E-27) +
					  pow(x_loc, 4) * pow(y_loc, 6) *
						  (-9.98E-29) +
					  pow(x_loc, 0) * pow(y_loc, 5) *
						  (-1.58E-16) +
					  pow(x_loc, 1) * pow(y_loc, 5) *
						  (-4.06E-20) +
					  pow(x_loc, 2) * pow(y_loc, 5) *
						  (-2.08E-21) +
					  pow(x_loc, 3) * pow(y_loc, 5) *
						  (1.95E-24) +
					  pow(x_loc, 4) * pow(y_loc, 5) *
						  (5.06E-26) +
					  pow(x_loc, 5) * pow(y_loc, 5) *
						  (2.13E-31) +
					  pow(x_loc, 0) * pow(y_loc, 4) *
						  (-2.79E-13) +
					  pow(x_loc, 1) * pow(y_loc, 4) *
						  (-1.51E-16) +
					  pow(x_loc, 2) * pow(y_loc, 4) *
						  (-7.75E-18) +
					  pow(x_loc, 3) * pow(y_loc, 4) *
						  (8.15E-21) +
					  pow(x_loc, 4) * pow(y_loc, 4) *
						  (2.12E-22) +
					  pow(x_loc, 5) * pow(y_loc, 4) *
						  (6.35E-26) +
					  pow(x_loc, 6) * pow(y_loc, 4) *
						  (1.11E-27) +
					  pow(x_loc, 0) * pow(y_loc, 3) *
						  (6.73E-11) +
					  pow(x_loc, 1) * pow(y_loc, 3) *
						  (6.45E-14) +
					  pow(x_loc, 2) * pow(y_loc, 3) *
						  (3.30E-15) +
					  pow(x_loc, 3) * pow(y_loc, 3) *
						  (-5.12E-18) +
					  pow(x_loc, 4) * pow(y_loc, 3) *
						  (-1.32E-19) +
					  pow(x_loc, 5) * pow(y_loc, 3) *
						  (5.34E-23) +
					  pow(x_loc, 6) * pow(y_loc, 3) *
						  (9.36E-25) +
					  pow(x_loc, 7) * pow(y_loc, 3) *
						  (9.94E-30) +
					  pow(x_loc, 0) * pow(y_loc, 2) *
						  (1.38E-07) +
					  pow(x_loc, 1) * pow(y_loc, 2) *
						  (7.30E-11) +
					  pow(x_loc, 2) * pow(y_loc, 2) *
						  (3.70E-12) +
					  pow(x_loc, 3) * pow(y_loc, 2) *
						  (-8.19E-15) +
					  pow(x_loc, 4) * pow(y_loc, 2) *
						  (-2.11E-16) +
					  pow(x_loc, 5) * pow(y_loc, 2) *
						  (-1.88E-20) +
					  pow(x_loc, 6) * pow(y_loc, 2) *
						  (-4.17E-22) +
					  pow(x_loc, 7) * pow(y_loc, 2) *
						  (-7.99E-25) +
					  pow(x_loc, 8) * pow(y_loc, 2) *
						  (-9.08E-27) +
					  pow(x_loc, 0) * pow(y_loc, 1) *
						  (7.76E-05) +
					  pow(x_loc, 1) * pow(y_loc, 1) *
						  (-3.91E-08) +
					  pow(x_loc, 2) * pow(y_loc, 1) *
						  (-2.04E-09) +
					  pow(x_loc, 3) * pow(y_loc, 1) *
						  (-1.42E-12) +
					  pow(x_loc, 4) * pow(y_loc, 1) *
						  (-3.55E-14) +
					  pow(x_loc, 5) * pow(y_loc, 1) *
						  (1.14E-16) +
					  pow(x_loc, 6) * pow(y_loc, 1) *
						  (1.92E-18) +
					  pow(x_loc, 7) * pow(y_loc, 1) *
						  (-6.16E-21) +
					  pow(x_loc, 8) * pow(y_loc, 1) *
						  (-8.47E-23) +
					  pow(x_loc, 9) * pow(y_loc, 1) *
						  (-8.37E-27) +
					  pow(x_loc, 0) * pow(y_loc, 0) *
						  (0.079182292) +
					  pow(x_loc, 1) * pow(y_loc, 0) *
						  (8.15E-05) +
					  pow(x_loc, 2) * pow(y_loc, 0) *
						  (4.30E-06) +
					  pow(x_loc, 3) * pow(y_loc, 0) *
						  (7.06E-09) +
					  pow(x_loc, 4) * pow(y_loc, 0) *
						  (2.05E-10) +
					  pow(x_loc, 5) * pow(y_loc, 0) *
						  (1.22E-12) +
					  pow(x_loc, 6) * pow(y_loc, 0) *
						  (1.99E-14) +
					  pow(x_loc, 7) * pow(y_loc, 0) *
						  (-4.46E-17) +
					  pow(x_loc, 8) * pow(y_loc, 0) *
						  (-5.61E-19) +
					  pow(x_loc, 9) * pow(y_loc, 0) *
						  (1.08E-22) +
					  pow(x_loc, 10) * pow(y_loc, 0) *
						  (8.35E-25);
		...
		Scales wrong*/
				C_UDMI(c, t, 4) = ((0.5 - cellpor) < 0) ?
							  0 :
							  (0.5 - cellpor);
			} else {
				C_UDMI(c, t, 4) = 0;
			}
			if (C_UDMI(c, t, 4) > maximum_vsi) {
				C_UDMI(c, t, 4) = maximum_vsi;
			}
		}
		end_c_loop(c, t)
	}
...
DEFINE_PROFILE(set_legacy_porosity_wachel, t, nv)
{
	...
	{
		C_CENTROID(x, c, t);
		x_loc = (fabs(x[0] - panelxoffset) / panelwidth * 166.88) -
			9.625; /* headgate 147.63, tailgate at 166.88  */
		y_loc = ((x[1] - panelyoffset) / (panelength / 2) * 1474.3) +
			1465.65; /* recovery 1474.3, startup at -1457 */
		if (ite <= 1) {
			cellpor =
				pow(x_loc, 1) * pow(y_loc, 9) * (-4.11E-36) +
				pow(x_loc, 0) * pow(y_loc, 10) * (6.35E-32) +
				pow(x_loc, 0) * pow(y_loc, 9) * (-3.66E-29) +
				pow(x_loc, 0) * pow(y_loc, 8) * (-2.57E-25) +
				pow(x_loc, 1) * pow(y_loc, 8) * (-3.02E-29) +
				pow(x_loc, 2) * pow(y_loc, 8) * (-1.57E-30) +
				pow(x_loc, 0) * pow(y_loc, 7) * (1.32E-22) +
				pow(x_loc, 1) * pow(y_loc, 7) * (8.23E-27) +
				pow(x_loc, 2) * pow(y_loc, 7) * (4.25E-28) +
				pow(x_loc, 3) * pow(y_loc, 7) * (-1.54E-33) +
				pow(x_loc, 0) * pow(y_loc, 6) * (3.95E-19) +
				pow(x_loc, 1) * pow(y_loc, 6) * (1.16E-22) +
				pow(x_loc, 2) * pow(y_loc, 6) * (6.00E-24) +
				pow(x_loc, 3) * pow(y_loc, 6) * (-3.85E-27) +
				pow(x_loc, 4) * pow(y_loc, 6) * (-9.98E-29) +
				pow(x_loc, 0) * pow(y_loc, 5) * (-1.58E-16) +
				pow(x_loc, 1) * pow(y_loc, 5) * (-4.06E-20) +
				pow(x_loc, 2) * pow(y_loc, 5) * (-2.08E-21) +
				pow(x_loc, 3) * pow(y_loc, 5) * (1.95E-24) +
				pow(x_loc, 4) * pow(y_loc, 5) * (5.06E-26) +
				pow(x_loc, 5) * pow(y_loc, 5) * (2.13E-31) +
				pow(x_loc, 0) * pow(y_loc, 4) * (-2.79E-13) +
				pow(x_loc, 1) * pow(y_loc, 4) * (-1.51E-16) +
				pow(x_loc, 2) * pow(y_loc, 4) * (-7.75E-18) +
				pow(x_loc, 3) * pow(y_loc, 4) * (8.15E-21) +
				pow(x_loc, 4) * pow(y_loc, 4) * (2.12E-22) +
				pow(x_loc, 5) * pow(y_loc, 4) * (6.35E-26) +
				pow(x_loc, 6) * pow(y_loc, 4) * (1.11E-27) +
				pow(x_loc, 0) * pow(y_loc, 3) * (6.73E-11) +
				pow(x_loc, 1) * pow(y_loc, 3) * (6.45E-14) +
				pow(x_loc, 2) * pow(y_loc, 3) * (3.30E-15) +
				pow(x_loc, 3) * pow(y_loc, 3) * (-5.12E-18) +
				pow(x_loc, 4) * pow(y_loc, 3) * (-1.32E-19) +
				pow(x_loc, 5) * pow(y_loc, 3) * (5.34E-23) +
				pow(x_loc, 6) * pow(y_loc, 3) * (9.36E-25) +
				pow(x_loc, 7) * pow(y_loc, 3) * (9.94E-30) +
				pow(x_loc, 0) * pow(y_loc, 2) * (1.38E-07) +
				pow(x_loc, 1) * pow(y_loc, 2) * (7.30E-11) +
				pow(x_loc, 2) * pow(y_loc, 2) * (3.70E-12) +
				pow(x_loc, 3) * pow(y_loc, 2) * (-8.19E-15) +
				pow(x_loc, 4) * pow(y_loc, 2) * (-2.11E-16) +
				pow(x_loc, 5) * pow(y_loc, 2) * (-1.88E-20) +
				pow(x_loc, 6) * pow(y_loc, 2) * (-4.17E-22) +
				pow(x_loc, 7) * pow(y_loc, 2) * (-7.99E-25) +
				pow(x_loc, 8) * pow(y_loc, 2) * (-9.08E-27) +
				pow(x_loc, 0) * pow(y_loc, 1) * (7.76E-05) +
				pow(x_loc, 1) * pow(y_loc, 1) * (-3.91E-08) +
				pow(x_loc, 2) * pow(y_loc, 1) * (-2.04E-09) +
				pow(x_loc, 3) * pow(y_loc, 1) * (-1.42E-12) +
				pow(x_loc, 4) * pow(y_loc, 1) * (-3.55E-14) +
				pow(x_loc, 5) * pow(y_loc, 1) * (1.14E-16) +
				pow(x_loc, 6) * pow(y_loc, 1) * (1.92E-18) +
				pow(x_loc, 7) * pow(y_loc, 1) * (-6.16E-21) +
				pow(x_loc, 8) * pow(y_loc, 1) * (-8.47E-23) +
				pow(x_loc, 9) * pow(y_loc, 1) * (-8.37E-27) +
				pow(x_loc, 0) * pow(y_loc, 0) * (0.079182292) +
				pow(x_loc, 1) * pow(y_loc, 0) * (8.15E-05) +
				pow(x_loc, 2) * pow(y_loc, 0) * (4.30E-06) +
				pow(x_loc, 3) * pow(y_loc, 0) * (7.06E-09) +
				pow(x_loc, 4) * pow(y_loc, 0) * (2.05E-10) +
				pow(x_loc, 5) * pow(y_loc, 0) * (1.22E-12) +
				pow(x_loc, 6) * pow(y_loc, 0) * (1.99E-14) +
				pow(x_loc, 7) * pow(y_loc, 0) * (-4.46E-17) +
				pow(x_loc, 8) * pow(y_loc, 0) * (-5.61E-19) +
				pow(x_loc, 9) * pow(y_loc, 0) * (1.08E-22) +
				pow(x_loc, 10) * pow(y_loc, 0) * (8.35E-25);
...
This equation is valid for use in the momentum conservation equation used by Ansys, Inc. in FLUENT. */
	...
	double cellporo;
	double initial_inertia_resistance;
	double cellinertiaresist;

	double V_v = 0.40000000;
	double a = 1;
	double resist_scaler = 1;
	double maximum_inertia_resist = 1.3E5;
	double minimum_inertia_resist = 0.000;
	if (RP_Variable_Exists_P(
...
This equation is valid for use in the momentum conservation equation used by Ansys, Inc. in FLUENT. */
	...
	cell_t c;
	double cellporo;
	double initial_permeability;
	double cellresist;

	double V_v = 0.40000000;
	double a = 1;
	double resist_scaler = 1;
	double maximum_resist = 5.000000E6;
	double minimum_resist =
...
DEFINE_PROFILE(set_legacy_perm_1_worrall, t, nv)
{
	...
	begin_c_loop(c, t)
	{
		...
		if (ite <= 1) {
			cellperm =
				(x_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 (-1.144063718331E-63)) +
				(1.000000000 * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * (1.667803992896E-60)) +
				(1.000000000 * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * (-6.751892520015E-57)) +
				(1.000000000 * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 (-9.474252626400E-54)) +
				(x_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc *
				 (-3.978169607429E-57)) +
				(x_loc * x_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 (-2.065804148485E-58)) +
				(1.000000000 * y_loc * y_loc * y_loc * y_loc *
				...
				(x_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc *
				 (3.778848307692E-54)) +
				(x_loc * x_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc *
				 (1.957503485522E-55)) +
				(x_loc * x_loc * x_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 (6.313487364221E-62)) +
				(1.000000000 * y_loc * y_loc * y_loc * y_loc *
				...
				(x_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * (3.336557503800E-50)) +
				(x_loc * x_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc *
				 (1.729578958791E-51)) +
				(x_loc * x_loc * x_loc * y_loc * y_loc * y_loc *
				 ...
				 y_loc * y_loc * y_loc * y_loc * y_loc *
				 (-2.804899223699E-55)) +
				(x_loc * x_loc * x_loc * x_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 (-7.269395530980E-57)) +
				(1.000000000 * y_loc * y_loc * y_loc * y_loc *
				...
				(x_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * (-3.199498959994E-47)) +
				(x_loc * x_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc *
				 (-1.656895162522E-48)) +
				...
				(x_loc * x_loc * x_loc * x_loc * y_loc * y_loc *
				 ...
				 y_loc * y_loc * y_loc * y_loc * y_loc *
				 (8.801298429234E-54)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 (4.934273626696E-59)) +
				(1.000000000 * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * (7.561292185998E-42)) +
				(x_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * (-1.128508835069E-43)) +
				(x_loc * x_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * (-5.839327957889E-45)) +
				(x_loc * x_loc * x_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * (1.869239188620E-48)) +
				(x_loc * x_loc * x_loc * x_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc *
				 (4.831470006198E-50)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * y_loc *
				 ...
				 y_loc * y_loc * y_loc * y_loc * y_loc *
				 (-5.038332937139E-54)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 (-8.607065333527E-56)) +
				(1.000000000 * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * (2.816664532243E-37)) +
				(x_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 (1.076974643570E-40)) +
				(x_loc * x_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * (5.568594890340E-42)) +
				(x_loc * x_loc * x_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * (-2.402361583225E-45)) +
				(x_loc * x_loc * x_loc * x_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc *
				 (-6.242748009406E-47)) +
				...
				(x_loc * x_loc * x_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * (-4.340939184349E-42)) +
				(x_loc * x_loc * x_loc * x_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * (-1.125103504664E-43)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc *
				 (-2.200092681492E-47)) +
				...
				(x_loc * x_loc * x_loc * x_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * (1.690234452480E-40)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * (-7.778380121475E-44)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc *
				 (-1.364002690849E-45)) +
				...
				(x_loc * x_loc * x_loc * x_loc * x_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * (2.957834678024E-40)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * (4.831892444390E-42)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc *
				 (-1.398510867623E-44)) +
				...
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 ...
				 (1.822822081644E-51)) +
				(1.000000000 * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * (1.281170237010E-25)) +
				(x_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * (1.678929156272E-28)) +
				(x_loc * x_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * (8.641048365500E-30)) +
				(x_loc * x_loc * x_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc *
				 (-8.577019706256E-33)) +
				(x_loc * x_loc * x_loc * x_loc * y_loc * y_loc *
				...
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 y_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * (3.163582495995E-39)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * (-3.772678383860E-42)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc *
				 (-4.810059899195E-44)) +
				...
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 ...
				 (8.687034882690E-54)) +
				(1.000000000 * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * (-3.709132151356E-23)) +
				(x_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * (8.613845618885E-26)) +
				(x_loc * x_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * (4.482571423358E-27)) +
				(x_loc * x_loc * x_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * (2.282138723322E-30)) +
				(x_loc * x_loc * x_loc * x_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc *
				 (4.697116932588E-32)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * y_loc *
				...
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * (4.630781762223E-38)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * (5.748458488688E-40)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * x_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc *
				 (-9.343654020395E-43)) +
				...
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 ...
				 (3.666910960522E-50)) +
				(1.000000000 * y_loc * y_loc * y_loc * y_loc *
				 y_loc * (-3.139384967068E-20)) +
				(x_loc * y_loc * y_loc * y_loc * y_loc * y_loc *
				 (-7.912082447522E-23)) +
				(x_loc * x_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * (-4.056561791628E-24)) +
				(x_loc * x_loc * x_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * (5.802557954940E-27)) +
				(x_loc * x_loc * x_loc * x_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc * (1.470918203378E-28)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * y_loc *
				 y_loc * y_loc * y_loc * y_loc *
				 (-2.648692282812E-31)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				...
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * y_loc * y_loc * y_loc * y_loc *
				 y_loc * (1.727290781194E-37)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * x_loc * y_loc * y_loc * y_loc *
				 y_loc * y_loc * (-4.805345611340E-40)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * x_loc * x_loc * y_loc * y_loc *
				 y_loc * y_loc * y_loc *
				 (-4.824682684994E-42)) +
				...
				(1.000000000 * y_loc * y_loc * y_loc * y_loc *
				 (9.268964018621E-18)) +
				(x_loc * y_loc * y_loc * y_loc * y_loc *
				 (-1.531377997083E-20)) +
				(x_loc * x_loc * y_loc * y_loc * y_loc * y_loc *
				 (-8.320722489547E-22)) +
				(x_loc * x_loc * x_loc * y_loc * y_loc * y_loc *
				 y_loc * (-4.314812461407E-24)) +
				(x_loc * x_loc * x_loc * x_loc * y_loc * y_loc *
				 y_loc * y_loc * (-9.847091988918E-26)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * y_loc *
				 y_loc * y_loc * y_loc * (8.689416045690E-28)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 y_loc * y_loc * y_loc * y_loc *
				 (1.378314356577E-29)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				...
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * x_loc * y_loc * y_loc * y_loc *
				 y_loc * (1.484092300195E-36)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * x_loc * x_loc * y_loc * y_loc *
				 y_loc * y_loc * (1.492298353485E-38)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * x_loc * x_loc * x_loc * y_loc *
				 y_loc * y_loc * y_loc *
				 (-1.428787551742E-41)) +
				...
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 ...
				 x_loc * x_loc * y_loc * y_loc * y_loc * y_loc *
				 (6.773231751144E-49)) +
				(1.000000000 * y_loc * y_loc * y_loc *
				 (3.190190706242E-15)) +
				(x_loc * y_loc * y_loc * y_loc *
				 (1.702022673981E-17)) +
				(x_loc * x_loc * y_loc * y_loc * y_loc *
				 (8.655054667145E-19)) +
				(x_loc * x_loc * x_loc * y_loc * y_loc * y_loc *
				 (-2.011063985918E-21)) +
				(x_loc * x_loc * x_loc * x_loc * y_loc * y_loc *
				 y_loc * (-4.892252477842E-23)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * y_loc *
				 y_loc * y_loc * (2.162513313402E-25)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 y_loc * y_loc * y_loc * (3.307183816977E-27)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * y_loc * y_loc * y_loc *
				 (-1.996125697995E-29)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				...
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * x_loc * x_loc * y_loc * y_loc *
				 y_loc * (9.943195129374E-36)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * x_loc * x_loc * x_loc * y_loc *
				 y_loc * y_loc * (-2.866641744039E-38)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 y_loc * y_loc * y_loc *
				 (-2.367856259851E-40)) +
				...
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 ...
				 x_loc * x_loc * x_loc * y_loc * y_loc * y_loc *
				 (4.538294541393E-50)) +
				(1.000000000 * y_loc * y_loc *
				 (-6.407634112118E-13)) +
				(x_loc * y_loc * y_loc *
				 (-2.332834112086E-17)) +
				(x_loc * x_loc * y_loc * y_loc *
				 (1.417237426917E-17)) +
				(x_loc * x_loc * x_loc * y_loc * y_loc *
				 (1.652990830539E-18)) +
				(x_loc * x_loc * x_loc * x_loc * y_loc * y_loc *
				 (3.725381796742E-20)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * y_loc *
				 y_loc * (-3.511945028423E-22)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 y_loc * y_loc * (-5.216876752286E-24)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * y_loc * y_loc * (3.749625814717E-26)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * y_loc * y_loc *
				 (4.035467912727E-28)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				...
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * x_loc * x_loc * x_loc * y_loc *
				 y_loc * (1.568770944843E-34)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 y_loc * y_loc * (1.139941145853E-36)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * y_loc * y_loc *
				 (-5.162933096051E-39)) +
				...
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 ...
				 x_loc * x_loc * x_loc * x_loc * y_loc * y_loc *
				 (4.345574550180E-46)) +
				(1.000000000 * y_loc * (4.403281264204E-12)) +
				(x_loc * y_loc * (-9.848089396826E-13)) +
				(x_loc * x_loc * y_loc *
				 (-4.794835509428E-14)) +
				(x_loc * x_loc * x_loc * y_loc *
				 (3.326085117662E-16)) +
				(x_loc * x_loc * x_loc * x_loc * y_loc *
				 (7.172899785535E-18)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * y_loc *
				 (-8.875294141709E-20)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 y_loc * (-1.146771046747E-21)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * y_loc * (1.679233473355E-23)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * y_loc * (1.680714341851E-25)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * x_loc * y_loc *
				 (-1.683437566112E-27)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				...
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 y_loc * (6.943142790520E-34)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * y_loc * (-2.516221184873E-36)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * y_loc *
				 (-1.745325683125E-38)) +
				...
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 ...
				 x_loc * x_loc * x_loc * x_loc * x_loc * y_loc *
				 (-2.341755164527E-48)) +
				(1.000000000 * 1 * (2.625441308571E-08)) +
				(x_loc * 1.000000000 * (1.750644880639E-10)) +
				(x_loc * x_loc * 1.000000000 *
				 (7.639753260973E-12)) +
				(x_loc * x_loc * x_loc * 1.000000000 *
				 (-1.449441737249E-13)) +
				(x_loc * x_loc * x_loc * x_loc * 1.000000000 *
				 (-2.605617501914E-15)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc *
				 1.000000000 * (6.867123803478E-17)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 1.000000000 * (8.506554922799E-19)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * 1.000000000 * (-1.442922691883E-20)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * 1.000000000 *
				 (-1.419604613451E-22)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				...
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * x_loc * x_loc * x_loc *
				 1.000000000 * (-4.454723463264E-29)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 1.000000000 * (-5.835270876962E-31)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * 1.000000000 * (-4.199071462702E-33)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * 1.000000000 *
				 (-1.384323250043E-35)) +
				...
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * x_loc * x_loc * x_loc *
				 1.000000000 * (-6.998238559677E-42)) +
				(x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 x_loc * x_loc * x_loc * x_loc * x_loc * x_loc *
				 1.000000000 * (-4.035548380822E-44));
			if (cellperm > 2.000E-7) {
				if (cellperm > 6.91E-6) {
					cellresist = 1.45E5;
				} else {
					cellresist = 1.0 / cellperm;
				}
			} else {
				cellresist = 5.000000E6;
			}
...
DEFINE_ON_DEMAND(calc_explosive_mix_2014)
{
	...
	thread_loop_c(t, d)
	{
		begin_c_loop(c, t)
		{
			...
			MW_N2 = 28.0134;
			MW_Mix = 1 /
				 (Y_CH4 / MW_CH4 + Y_O2 / MW_O2 + Y_N2 / MW_N2);
			X_CH4 = (Y_CH4 * MW_Mix) /
				MW_CH4; /* X = Mole Fraction of X */
			X_O2 = (Y_O2 * MW_Mix) / MW_O2;
			px = X_CH4;
			py = X_O2;
			u = 0.8529 * px +
			    0.0606; /* Near Explosive to Explosive Slope */
			v = -0.21 * px + 0.21; /* Upper Explosive Limit  */
			u1 = 0.8864 * px +
			     0.0445; /* Near Explosive to Requires Air Slope */
			v1 = -1.3929 * px + 0.195;
...
For strata use: calc_explosive_integral*/
{
	...
	thread_loop_c(t, d)
	{
		begin_c_loop(c, t)
		{
			if (C_UDMI(c, t, 2) > 0.99e0 &&
			    C_UDMI(c, t, 2) < 1.01) {
				/* Assign marker value for cell volume that is explosive */

				C_UDMI(c, t, 3) = 1.00E0 * C_UDMI(c, t, 1);
...
double SUB_CRITICAL_Trona_RECOVERY_CORNER(double x_loc, double y_loc)
{
	...
	FUN = pow((x_loc * y_loc), 0.107302089705487) *
	      (0.1477 -
	       0.812278751377339 * exp(-9.96978304250904 * y_loc) * y_loc *
		       x_loc +
	       0.103507270969929 * exp(-688.057090793680 * y_loc * x_loc) -
	       0.1738 * exp(-6.368 * y_loc) +
	       0.1971 * x_loc * x_loc * exp(-1.38 * x_loc * x_loc) +
	       13.6 * y_loc * y_loc * exp(-2890 * y_loc) -
	       14.56 * x_loc * exp(-47.01 * x_loc) +
	       11.19 * x_loc * exp(-7883 * x_loc * x_loc) +
	       0.07992 * exp(-2.155 * x_loc) -
	       6.274 * y_loc * exp(-99.58 * y_loc) +
	       0.03141 * y_loc * exp(-8.748 * y_loc * y_loc));
...
double SUB_CRITICAL_Trona_CENTER_GATEROAD(double x_loc)
{
	...
	FUN = 0.2031 + 0.007304 * x_loc + 1.495 * x_loc * exp(-19.69 * x_loc) -
	      0.1661 * exp(-162.6 * x_loc * x_loc) -
	      0.1315 * x_loc * exp(-7.204 * x_loc * x_loc) -
	      3.298 * x_loc * x_loc * exp(-44.01 * x_loc * x_loc);
...
double SUB_CRITICAL_Trona_STARTUP_CORNER(double x_loc, double y_loc)
{
	...
	FUN = pow((x_loc * y_loc), 0.1007) *
	      (0.1796 + 0.2762 * exp(-4.552 * y_loc) * y_loc * x_loc +
	       0.04375 * exp(-5.354 * x_loc * y_loc) -
	       0.3093 * exp(-60.54 * x_loc) - 0.2702 * exp(-50.36 * y_loc) +
	       0.437 * x_loc * x_loc * exp(-2.728 * x_loc));
...
{
	double FUN = 0.0;
	FUN = 0.155394214 + x_loc * x_loc * (-0.004966014) +
	      0.142894504 * y_loc * exp(-1.11507158 * y_loc * y_loc) -
	      0.154156852 * exp(-994.6190264 * y_loc * y_loc) -
	      0.165429282 * y_loc * y_loc * exp(-2.119029131 * y_loc * y_loc);
...
{
	double FUN = 0.0;
	FUN = 0.182881808 + 0.000219076 * y_loc - 0.001701901 * x_loc * x_loc -
	      0.003415753 * x_loc * x_loc * x_loc;
...
{
	double FUN = 0.0;
	FUN = 0.034705045 + x_loc * x_loc * (-0.007156676) +
	      0.392853454 * y_loc * exp(-2.690847002 * y_loc * y_loc) -
	      0.016570035 * exp(-290 * y_loc * y_loc) +
	      0.206091545 * y_loc * y_loc * exp(-0.513740978 * y_loc * y_loc);
...
{
	double FUN = 0.0;
	FUN = pow((x_loc * y_loc), 0.070680995) *
	      (0.162003881 +
	       0.114056257 * exp(-2.060750448 * y_loc) * y_loc * x_loc +
	       0.027309527 * exp(-2.32002878 * x_loc * y_loc) -
	       0.134663756 * exp(-8.323851432 * x_loc) -
	       0.263467643 * exp(-50.02086538 * y_loc) +
	       51.01309648 * x_loc * x_loc * exp(-24.66420708 * x_loc));
...
{
	double FUN = 0.0;
	FUN = 0.10083973 + 0.05329973 * x_loc + 0.000111875 * y_loc +
	      0.715710581 * x_loc * exp(-3.193027724 * x_loc) -
	      0.100070375 * exp(-1200.384929 * x_loc * x_loc) -
	      0.151653961 * x_loc * exp(-3.716593738 * x_loc * x_loc) -
	      0.378856069 * x_loc * x_loc * exp(-16.20732696 * x_loc * x_loc);
...
{
	double FUN = 0.0;
	FUN = pow((x_loc * y_loc), 0.251307505) *
	      (0.197539477 -
	       0.258183405 * exp(-2.062155525 * y_loc) * y_loc * x_loc +
	       0.02301539 * exp(-21.41498958 * y_loc * x_loc) -
	       0.15928258 * exp(-10.01527015 * y_loc) +
	       0.445654501 * x_loc * x_loc * exp(-17.13983263 * x_loc * x_loc) +
	       4.68818221 * y_loc * y_loc * exp(-5.633256844 * y_loc) -
	       13.90840849 * x_loc * exp(-65.9273908 * x_loc) +
	       0.772026679 * x_loc * exp(-68.99785585 * x_loc * x_loc) +
	       34.5 * exp(-3200.000001 * x_loc) +
	       0.263621861 * y_loc * exp(-27.16257242 * y_loc) -
	       0.255066042 * y_loc * exp(-9.010678902 * y_loc * y_loc));
...
{
	double FUN = 0.0;
	FUN = 0.23446547 + x_loc * x_loc * (-0.007274502) +
	      0.21112871 * y_loc * exp(-1.254341353 * y_loc * y_loc) -
	      0.232563013 * exp(-986.7723584 * y_loc * y_loc) -
	      0.288213205 * y_loc * y_loc * exp(-2.41029839 * y_loc * y_loc);
...
{
	double FUN = 0.0;
	FUN = 0.26928324 + 0.000607666 * y_loc - 0.001387445 * x_loc * x_loc -
	      0.005923021406 * x_loc * x_loc * x_loc;
	/*  CHECK  Center Panel */
	/* p00  + p01*y  +  p20*x^2 + p30*x^3					*/
	/* 0.26928324	0.000607666	-0.001387445		*/
	FUN = (FUN < 0) ? 0 : FUN;
...
{
	double FUN = 0.0;
	FUN = 0.054623275 + x_loc * x_loc * (-0.007156676) +
	      0.537305093 * y_loc * exp(-3.725760322 * y_loc * y_loc) -
	      0.028010127 * exp(-290 * y_loc * y_loc) +
	      0.501978887 * y_loc * y_loc * exp(-0.911642577 * y_loc * y_loc);
...
{
	double FUN = 0.0;
	FUN = pow((x_loc * y_loc), 0.162024335) *
	      (0.262664371 -
	       0.253166473007042 * exp(-3.203358282 * y_loc) * y_loc * x_loc +
	       0.065491826 * exp(-228.3897538 * y_loc * x_loc) -
	       0.243491669 * exp(-8.890275525 * y_loc) -
	       0.01 * x_loc * x_loc * exp(-47.29743004 * x_loc * x_loc) +
	       5.007983398 * y_loc * y_loc * exp(-5.70033048 * y_loc) -
	       27.10582475 * x_loc * exp(-73.60496053 * x_loc) +
	       0.315580701 * x_loc * exp(-51.89579568 * x_loc * x_loc) +
	       38.98661392 * exp(-3169.255209 * x_loc) +
	       2.089424053 * y_loc * exp(-30.00980383 * y_loc) -
	       0.303675247 * y_loc * exp(-7.38256233 * y_loc * y_loc));
...
{
	double FUN = 0.0;
	FUN = pow((x_loc * y_loc), 0.072583782) *
	      (0.222803703 +
	       0.217897624 * exp(-3.019723703 * y_loc) * y_loc * x_loc +
	       0.035000529 * exp(-4.519932999 * x_loc * y_loc) -
	       0.213737696 * exp(-33.3990023 * x_loc) -
	       0.399728137 * exp(-49.33761923 * y_loc) +
	       0.36279155 * x_loc * x_loc * exp(-2.519206805 * x_loc));
...
{
	double FUN = 0.0;
	FUN = 0.10083973 + 0.128284224 * x_loc + 0.000603995 * y_loc +
	      2.171935712 * x_loc * exp(-4.062177714 * x_loc) -
	      0.101220528 * exp(-1464.235434 * x_loc * x_loc) -
	      0.474820214 * x_loc * exp(-5.389192365 * x_loc * x_loc) -
	      1.477162806 * x_loc * x_loc * exp(-23.91528913 * x_loc * x_loc);
...
void Print_Scheme_Variable_Settings()
{
	if (RP_Variable_Exists_P(
		...
		Message("Porosity scaler: %g\n",
			RP_Get_Real("vsi/porosity-scaler"));
	} /* else default or manual set above is used */
	else {
	...
	if (RP_Variable_Exists_P(
		...
		Message("The starting maximum porosity behind shields starts at: %g\n",
			RP_Get_Real("vsi/maximum-porosity"));
	} /* else default or manual set above is used */
	else {
	...
	if (RP_Variable_Exists_P(
		...
		Message("Initial Porosity of host rock is set to: %g\n",
			RP_Get_Real("vsi/initial-porosity"));
	} /* else default or manual set above is used */
	else {
	...
	if (RP_Variable_Exists_P(
		...
		Message("Maximum cropped resistance (before scaler) in the gob center is set to: %g\n",
			RP_Get_Real("vsi/maximum-resist"));
	} /* else default or manual set above is used */
	else {
	...
	if (RP_Variable_Exists_P(
		...
		Message("Minimum cropped resistance (before scaler) in the edges of the gob is set to: %g\n",
			RP_Get_Real("vsi/minimum-resist"));
	} /* else default or manual set above is used */
	else {
